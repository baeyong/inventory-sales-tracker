...
import { bulkUnmarkInvestment } from "@/app/(app)/actions";
...
export default function InvestmentsTable({ items }: { items: Item[] }) {
  ...
  function moveBack(id: string) {
    ...
    startTransition(async () => {
      const res = await bulkUnmarkInvestment([id]);
        ...
        {visible.map((item) => (
          ...
          >
            ...
            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-sm text-zinc-600 dark:text-zinc-300">
              ...
              <span className="text-zinc-500">
                Moved {formatDate(item.invested_at)}
              </span>
            </div>
            <div className="mt-3">
              <button
        ...
        <table className="w-full min-w-[860px] text-sm">
          <thead className="bg-zinc-50 text-left text-xs uppercase tracking-wide text-zinc-500 dark:bg-zinc-900">
            <tr>
              ...
              <SortHeader label="Moved" sortKey="invested_at" activeKey={sortKey} dir={sortDir} onSort={toggleSort} />
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {visible.map((item) => (
              <tr key={item.id} className="bg-white dark:bg-zinc-950">
                ...
                <td className="px-4 py-3">{formatDate(item.invested_at)}</td>
                <td className="px-4 py-3 text-right">
                  <button
                    type="button"
